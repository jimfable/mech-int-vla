...
\paragraph{What we would do next, if anything.} Not another run of this design. The informative follow-ups are (i) the missing model, outputs\,+\,internals without simulator state, first on Calibration out-of-fold and, if promising, as an explicitly post-hoc Locked Test analysis; (ii) a cross-layer causal sweep with the same controls, cheap now that the pipeline exists ($\approx$\,3 GPU-hours per layer); and (iii) replacing the orientation probe with a probe for the variable the policy actually fails on (contact/grasp), which the per-cell results suggest.
